        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {programs.map((prog, i) => (
            ...
            >
              ...
              >
                <div className={`relative h-72 overflow-hidden ${prog.id === 'the-move' ? 'bg-black' : ''}`}>
                  <img
                    src={prog.image}
                    alt={prog.title}
                    className={`w-full h-full ${prog.id === 'the-move' ? 'object-cover object-center scale-[1.35] transition-transform duration-700 group-hover:scale-[1.42]' : 'object-cover transition-transform duration-700 group-hover:scale-105'}`}
                  />
                  {prog.id !== 'the-move' && <div className="absolute inset-0 bg-gradient-to-t from-slate-900 via-slate-900/20 to-transparent" />}
