...
// Exchange the admin passcode for a bearer token
app.post("/make-server-d6d8acf9/admin/login", async (c) => {
  try {
...
// Create submission (public — visitor forms)
app.post("/make-server-d6d8acf9/submissions", async (c) => {
  try {
    ...
    // Sanitize: strings only, capped length, capped field count
...
app.delete("/make-server-d6d8acf9/submissions", async (c) => {
  const authError = await checkAdmin(c);
  if (authError) return authError;
  try {
